...
export function HistorySidebar({
						...
						{filteredChats.map((chat) => {
							...
							return (
								<li key={chat.chatId}>
									<div
										className={cn(
											"group relative flex items-center gap-3 rounded-xl border px-2.5 py-2.5 transition-all duration-200",
											isActive
				...
				<div className="relative" ref={accountMenuRef}>
					...
					>
						<div className="relative shrink-0">
							<div className="size-8 rounded-xl border border-border/40 bg-muted/30 overflow-hidden shadow-sm">
								{userImage ? (
									<img
										src={userImage}
										alt={userName || ""}
	...
	if (variant === "docked") {
		return (
			<div className="h-full w-full min-w-0 overflow-hidden border-r border-border/70">
				{panel}
			</div>
		);
	}
